...
def extractLinksFromURL(url:str,tag:str,class_:str)-> list:
    ...
    # Find all anchor tags with a class of "card-header"
...
def getUdemyUrl(link:str)-> str:
    ...
    soup = BeautifulSoup(response.text, 'html.parser')
    elements = soup.find_all('a',id='couponLink')
...
def scrape(fast:bool=False,**kwargs) -> Generator:
    """Scrape coupons from `DiscUdemy.com`"""
    debug = kwargs['debug']
    collected = []
    valid=0

    debug.print(f"Extracting URLs from DiscUdemy")
    for i in range(1,2):
        debug.print(f"Extracting URLs from DiscUdemy page {i}...")
        results = extractLinksFromURL(f"https://www.discudemy.com/all/{i}",'a','card-header')
        collected.extend(results)
        debug.print(f"Got {len(results)}! New total: {len(collected)}")
    debug.is_summary(f"Total scraped from DiscUdemy: {len(collected)}")
    debug.print(f"\n\nExtracting URLs from DiscUdemy")

    validation_start = arrow.utcnow()
    yield {'mode':'init', 'message':'Started validating courses...', "count":{len(collected)}, 'provider': 'discudemy'}
